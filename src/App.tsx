...
function NavItem({ to, icon, label, color }: { to: string; icon: React.ReactNode; label: string; color?: string }) {
  return (
    <NavLink to={to} className={({ isActive }) => `nav-link${isActive ? ' active' : ''}`}
      style={color ? ({ isActive }) => isActive ? {} : { color } : undefined}>
...
function Sidebar() {
  ...
  const roleStyle = user ? (ROLE_STYLE[user.role] ?? ROLE_STYLE.user) : ROLE_STYLE.user;
  ...
  return (
    <aside className="sidebar">
      ...
      <NavItem to="/clients" icon={<I.clients />} label="Clients" />
      <NavItem to="/mods"    icon={<I.mods />}    label="Mods" />
      <NavItem to="/skins"   icon={<I.skins />}   label="Skins" />

      {isAuthenticated && (
        <>
          <div className="nav-section">Account</div>
          <NavItem to="/settings" icon={<I.settings />} label="Settings" />
          {canAdmin && (
            <NavItem to="/admin" icon={<I.admin />} label="Admin"
      ...
      {isAuthenticated && user ? (
        <div className="sidebar-user-wrap">
          ...
          <button className="nav-link logout-btn" onClick={async () => { await logout(); navigate('/login'); }}>
